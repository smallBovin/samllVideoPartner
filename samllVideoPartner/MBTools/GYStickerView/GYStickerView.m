...
- (void)setSelected:(BOOL)selected {
    _selected = selected;
    if (selected) {
        self.contentView.layer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"sticker_boarder"].CGImage);
        [self setRemoveCtrlImage:[UIImage imageNamed:@"delete_sticker"]];
        [self setTransformCtrlImage:[UIImage imageNamed:@"zoom_ratation"]];
    }else {
        self.contentView.layer.contents = nil;
        [self setRemoveCtrlImage:nil];
        [self setTransformCtrlImage:nil];
    }
}
...
- (void)rotateAroundOPointWithCtrlPoint:(CGPoint)ctrlPoint {
    ...
    self.rotationAngil = angle;
    NSLog(@"djksfkjdljd 旋转角度  %f",angle);
...
- (void)removeCtrlTap:(UITapGestureRecognizer *)gesture {
    if (self.DeleteStickerComplete) {
        self.DeleteStickerComplete(self);
    }
